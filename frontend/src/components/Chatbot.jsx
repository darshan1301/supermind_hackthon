...
import { Send, MoreVertical, CheckCheck, Image, Paperclip, Smile } from 'lucide-react';
...
const mockPostChatMessage = async (message) => {
  await new Promise(resolve => setTimeout(resolve, 1000));
  return {
    data: mockResponses[Math.floor(Math.random() * mockResponses.length)]
  };
};

const mockGetChatMessages = async () => {
...
const ChatBot = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await mockPostChatMessage({ text: userInput });
      const botReply = {
        sender: "bot",
        text: response.data,
        timestamp: new Date(),
  ...
  React.useEffect(() => {
    const fetchInitialMessages = async () => {
      try {
        const previousMessages = await mockGetChatMessages();
  ...
  return (
     <div className="flex flex-col h-[calc(100vh-4rem)] bg-gray-900 text-gray-100"> {/* Reduced height to accommodate navbar */}
     {/* Header */}
